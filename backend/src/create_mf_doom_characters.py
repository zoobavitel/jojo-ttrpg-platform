...
# 1. Find or Create a GM user
user_gm, created = User.objects.get_or_create(
    username="test_gm", defaults={"email": "test_gm@example.com"}
)
if created:
    user_gm.set_password(User.objects.make_random_password())
    user_gm.save()
    print("Created user 'test_gm'")
else:
    print("Using existing user 'test_gm'")

# Get existing Heritage and Abilities for PC
heritage_human = Heritage.objects.get_or_create(
    name="Human", defaults={"base_hp": 0, "description": "A standard human heritage"}
)[0]
ability1 = Ability.objects.get_or_create(
    name="Standard Ability 1",
    defaults={"type": "standard", "description": "A basic ability"},
)[0]
ability2 = Ability.objects.get_or_create(
    name="Standard Ability 2",
    defaults={"type": "standard", "description": "Another basic ability"},
)[0]
ability3 = Ability.objects.get_or_create(
    name="Standard Ability 3",
    defaults={"type": "standard", "description": "A third basic ability"},
)[0]
...
campaign_npc, created_campaign = Campaign.objects.get_or_create(
    name="NPC Default Campaign", defaults={"gm": user_gm}
)
if created_campaign:
