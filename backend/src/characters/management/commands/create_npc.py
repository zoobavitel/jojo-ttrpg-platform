...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "campaign_id", type=int, help="The ID of the campaign the NPC belongs to."
        )
        parser.add_argument(
            "creator_id", type=int, help="The ID of the user creating the NPC."
        )
        parser.add_argument(
            "--power",
            type=str,
            default="D",
            help="Stand Power rating (S, A, B, C, D, F).",
        )
        parser.add_argument(
            "--speed",
            type=str,
            default="D",
            help="Stand Speed rating (S, A, B, C, D, F).",
        )
        parser.add_argument(
            "--range",
            type=str,
            default="D",
            help="Stand Range rating (S, A, B, C, D, F).",
        )
        parser.add_argument(
            "--durability",
            type=str,
            default="D",
            help="Stand Durability rating (S, A, B, C, D, F).",
        )
        parser.add_argument(
            "--precision",
            type=str,
            default="F",
            help="Stand Precision rating (S, A, B, C, D, F).",
        )
        parser.add_argument(
            "--development",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'User with ID "{creator_id}" does not exist.')
            )
            return
        ...
        total_points = sum(
            grade_points.get(grade, 0) for grade in stand_coin_stats.values()
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully created NPC "{npc.name}" with ID {npc.id} at level {npc.level}.'
            )
        )
