...
export const useDiceRolling = (characterId, xpTracks, setXpTracks) => {
  ...
  const rollDice = async (
    ...
    try {
      ...
      console.error(
        "Backend dice roll failed, falling back to client-side:",
        error,
      );
  ...
  const markAsDesperateAction = () => {
    if (!diceResult || diceResult.isResistance || diceResult.isDesperateAction)
      return;
