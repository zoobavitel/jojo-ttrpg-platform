...
import { characterAPI, transformBackendToFrontend, transformFrontendToBackend } from '../features/character-sheet';
import { useAuth } from '../features/auth';
import CharacterSheetWrapper from './Character';
...
const HomePage = () => {
  ...
  const CharacterSheetModal = () => {
    ...
    // Use CharacterSheetWrapper (experimental design — all SRD fixes included)
    const frontendChar = selectedCharacter ? transformBackendToFrontend(selectedCharacter) : null;
    const handleSheetSave = async (frontendData) => {
      try {
        const backendData = transformFrontendToBackend(frontendData);
        if (frontendData.id && characters.some(c => c.id === frontendData.id)) {
          await characterAPI.updateCharacter(frontendData.id, backendData);
        } else {
          const created = await characterAPI.createCharacter(backendData);
          frontendData.id = created.id;
        }
        const refreshed = await characterAPI.getCharacters();
        setCharacters(refreshed);
      } catch (err) {
        console.error('Save failed, keeping local state:', err);
      }
    ...
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto">
        <CharacterSheetWrapper
          ...
          allCharacters={characters.map(transformBackendToFrontend)}
          ...
          onSwitchCharacter={(c) => {
            const found = characters.find(ch => ch.id === c.id);
